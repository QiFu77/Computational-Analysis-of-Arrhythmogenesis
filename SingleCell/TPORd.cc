...
public:
	...
	TPORd(CellType ct) // 0-ENDO 1-EPI 2-M
	{
		init(ct);
	}
	...
	void update()
	{
	 ...
	 //**********************************四种病理类型的膜电位表示        分别是100%IKr	50%IKr	25%IKr	0%IKr
		#ifdef WT
		IKr = IKr;
		#endif
		...
		//**************************************************************************************药理部分*************
		#ifdef ANTIBODY60
		double antibody_Num=60;
		double delta_V=0;
		delta_V = 120 / (1.0+pow(antibody_Num/118.0930,-1.7499));
		xs1ss=1.0/(1.0+exp((-(v+11.60+delta_V))/8.932));
		#endif

		#ifdef ANTIBODY300
		double antibody_Num=300;
		double delta_V=0;
		delta_V = 120 / (1.0+pow(antibody_Num/118.0930,-1.7499));
		xs1ss=1.0/(1.0+exp((-(v+11.60+delta_V))/8.932));
		#endif
